...
const exampleData = require("../../core-data");
...
describe("Node API end-to-end", () => {
  ...
  it("bundles to a directory", () => {
    return techRadarGenerator(exampleData, tmpDir.name, {
      mode: "development",
    }).then((dist) => {
      ...
      expect(
        outputFiles.findIndex((f) => f.match(/^common\..*\.css/))
      ).not.toBe(-1);
      expect(outputFiles.findIndex((f) => f.match(/^common\..*\.js/))).not.toBe(
        -1
      );
      expect(outputFiles.findIndex((f) => f.match(/^main\..*\.css/))).not.toBe(
        -1
      );
      expect(outputFiles.findIndex((f) => f.match(/^main\..*\.js/))).not.toBe(
        -1
      );
    });
  });
  ...
  it("fails when the data is bad", () => {
    return techRadarGenerator({ bad: "data" }, tmpDir.name, {
      mode: "development",
    })
      .catch((e) => {
